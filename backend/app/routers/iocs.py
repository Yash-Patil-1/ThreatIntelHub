...
SEV_RANK = {name: i for i, name in enumerate(SEVERITIES)}  # critical=0 ... info=4


def decode_cursor(cursor: str) -> tuple | None:
    try:
        sev, score, seen_iso, ioc_id = base64.urlsafe_b64decode(cursor.encode()).decode().split("|")
        return SEV_RANK[sev], int(score), datetime.fromisoformat(seen_iso), int(ioc_id)
...
def _keyset_after(cur: tuple):
    ...
    ORDER BY severity-rank ASC (worst-first), threat_score DESC, last_seen DESC, id ASC."""
    rank, ts, ls, cid = cur
    r = _sev_rank_expr()
    return or_(
        r > rank,
        (r == rank) & (Ioc.threat_score < ts),
        (r == rank) & (Ioc.threat_score == ts) & (Ioc.last_seen < ls),
        (r == rank) & (Ioc.threat_score == ts) & (Ioc.last_seen == ls) & (Ioc.id > cid),
    )


def _sev_rank_expr():
    from sqlalchemy import case
    return case(SEV_RANK, value=Ioc.severity)
...
async def _page_rows(db: AsyncSession, conds, limit: int):
    stmt = (
        select(Ioc)
        .where(*conds)
        .order_by(_sev_rank_expr().asc(), Ioc.threat_score.desc(), Ioc.last_seen.desc(), Ioc.id.asc())
        .limit(limit)
    )
